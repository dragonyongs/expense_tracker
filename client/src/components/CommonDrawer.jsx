...
const CommonDrawer = ({ isOpen, onClose, title,  children, className }) => {
    //const { setThemeColor, resetThemeColor, themeColor } = useTheme();color, darkColor,
    ...
    //const backgroundColor = isDarkMode && darkColor ? darkColor : isDarkMode ? "bg-slate-800" : ""; ${backgroundColor}

    return (
        <Drawer open={isOpen} onClose={onClose} className="h-real-screen" duration="300" direction="right" size={drawerSize}>
            <div
                className={`${className} flex justify-between py-2 px-4`}
