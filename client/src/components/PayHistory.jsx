...
const PayHistory = ({ transactions, userCards, isLoading }) => {
  ...
  const filteredTransactions = transactions.filter(
    (type) => type.transaction_type !== "income"
  );
  ...
  return (
    <div className="flex flex-col pb-20 xl:px-4">
      <div className="flex justify-between items-center mt-8 mb-2 px-6">
        ...
        <Link
          to={"/transactions"}
          className="flex items-center gap-x-1 text-slate-400"
        >
          전체 <RiArrowRightSLine />
        </Link>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-dashboard-screen">
          <MutatingDots
            ...
            wrapperClass=""
          />
          <p className="text-lg font-semibold text-blue-900 dark:text-gray-500">
            데이터를 불러오는 중입니다.
          </p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center h-dashboard-screen">
          <img
            src="/icon-wallet.png"
            alt="No Transactions"
            className="mb-4 w-32 h-32 dark:invert dark:opacity-40"
          />
            ...
            {filteredTransactions
              ...
              .map((transaction) => (
                <li
                  ...
                  onClick={() => handleOpenTransactionReceipt(transaction)}
                >
