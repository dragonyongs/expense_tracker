...
const AppravalPending = ({ data, onUpdateStatus, isEditing, isApprover = false, isLoading }) => {
    ...
    // 현재 사용자의 이름 (데모에서는 하드코딩, 실제 데이터라면 로그인 사용자 정보에서 가져와야 함)
    const currentApproverName = "고길동"; // 본부장 역할
    const currentStep = 2; // 현재 결재 단계

    // 승인 처리 로직
    const handleApprove = (id) => {
        onUpdateStatus(id, "approved", currentApproverName, currentStep); // 부모 상태 변경 함수 호출
        handleCloseDrawer();
    };
    ...
    const filteredData = data.filter(item => item.status !== "완료");
