...
const AdminDeposit = () => {
            ...
            <div>
                ...
                {['RegularDeposit', 'TransportationDeposit', 'TeamFund']
                    ...
                    .map((depositType, index) => {
                        const isChecked = state.selectedDeposit.menu_items.some(item => item.deposit_type === depositType);
                        const depositLabels = {
        ...
        <CommonDrawer isOpen={state.isDeductedOpen} onClose={handleCloseDrawer} title={state.isEditingMinus ? '출금 수정' : '출금 추가'}>
            ...
            {/* <div className="p-4 bg-slate-50 border border-slate-200 space-y-3">
                ...
                {state.menuItems.map((item, index) => (
                    <li key={index}>
                    <div className="flex justify-between">
                        <p className="text-sm text-slate-600">{item.name} <span className='font-semibold'>{item.price}</span>원</p>
                        <button className="text-sm text-red-600 underline">제거</button> */}
                        {/* onClick={() => handleRemoveMenuItem(index)} */}
                    {/* </div>
                    </li>
                ...
                <div className="text-base text-blue-600">총액: <span className='font-semibold'>{calculateTotalAmount()} 원</span></div>
            </div> */}
    ...
    //             merchant_name: "관리자",
    //             menu_items: [{
    //                 deposit_type: state.selectedDeposit?.deposit_type || 'RegularDeposit',
    ...
    //                     />
                        ...
                        //     <div className="flex justify-between gap-x-3 items-center">
                        //         <button className="w-1/2 py-2 px-4 rounded-lg bg-blue-600 text-white">추가</button>
                        //         <button className="w-1/2 py-2 px-4 rounded-lg bg-slate-600 text-white">취소</button>
                        //     </div>
