...
const AdminTeams = () => {
    ...
    const fetchTeams = async () => {
        try {
            const { data } = await axios.get(API_URLS.TEAMS);
            setTeams(data);
        } catch (error) {
    ...
    const handleAddTeam = () => {
        setSelectedTeam({ team_name: '', department_id: '' });
        setIsEditing(false);
        setIsOpen(true);
    };
    ...
    const groupedTeams = departments.reduce((grouped, department) => {
        ...
        <>
            ...
            <div className="flex-1 w-full p-4 sm:p-6 dark:bg-gray-800">
                <div className="flex items-center justify-between mt-2 mb-4 px-3">
                    <h5 className="text-xl font-bold text-gray-900 dark:text-white">팀 목록</h5>
                    <button
                        type="button"
                    ...
                    {teams.length === 0 ? (
                        ...
                        Object.keys(groupedTeams).map(departmentName => (
                            <div key={departmentName} className="space-y-4 bg-white p-4 rounded-lg shadow-sm dark:bg-gray-700 mb-4">
                                ...
                                {groupedTeams[departmentName].length === 0 ? (
                                    <p className="pb-3 text-center dark:text-slate-400">소속된 팀이 없습니다.</p>
                                ) : (
