...
function Teams() {
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-y-2 px-4 pb-24">
        {accounts.length === 0 ? (
          <NoCardMessage />
        ) : (
          <AccountList
  ...
  return (
    <>
      {accounts.map((account) => {
        const hasOvertimeMealCard = account.cards.some(
          (card) => card.card_type === "OvertimeMealCard"
        );
        const isLeaderAccount = account.cards.some(
          (card) => card.position === "팀장"
        );
        const isPartLeaderAccount = account.cards.some(
          (card) => card.position === "파트장"
        );
        const isUserAccount = account.cards.some((card) => {
          card.member_name === userName;
        });
...
const calculateTotalBalance = (cards) => {
  return cards.reduce(
    (sum, card) => sum + card.balance + card.rollover_amount + card.team_fund,
    0
  );
};
...
const AccountCard = ({ account, userPosition, remainingDays }) => {
  ...
  // 팀장 및 야근식대 카드 분리
  const leaderCards = account.cards.filter(
    (card) => card.position === "팀장" && card.card_type !== "OvertimeMealCard"
  );
  // const overtimeMealCards = account.cards.filter(card => card.card_type === "OvertimeMealCard");
  const overtimeMealCard = account.cards.find(
    (card) => card.card_type === "OvertimeMealCard"
  );
  const otherCards = account.cards.filter(
    (card) => card.position !== "팀장" && card.card_type !== "OvertimeMealCard"
  );
  ...
  return (
    <div className="pt-8 px-8 bg-white shadow-sm rounded-xl border-t dark:border dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="mt-8">
        ...
        {(userPosition === "팀장" || userPosition === "파트장") &&
...
const LeaderCardDetail = ({ leaderCards, teamMembersCount, remainingDays }) => {
  const totalBalance = leaderCards.reduce(
    (sum, card) => sum + card.balance + card.rollover_amount + card.team_fund,
    0
  );
  ...
  return (
    <div className="mb-10">
      ...
      {leaderCards.map((card) => (
        <div
          key={card.card_number}
          className="flex justify-between mt-4 pt-4 border-t dark:border-slate-600"
        >
...
const CardDetail = ({
  ...
  const remainingBalanceCriteria = totalBalance / teamMembersCount > 10000;
  const isWarning =
    !isOvertimeMealCard && remainingBalanceCriteria && remainingDays <= 7;
  const spentPercentage = ((card.limit - card.balance) / card.limit) * 100;
  const isMobile = useMediaQuery("(max-width: 640px)");
  const drawerSize = isMobile ? "100%" : "375px";
  ...
  const handleClick = async (cardId) => {
    try {
      ...
      const response = await axios.get(
        `${API_URLS.CARD_TRANSACTIONS}/${cardId}`
      );
      setSelectedTransactions(response.data);
    } catch (error) {
    ...
    <>
      <div className="mb-10" onClick={() => handleClick(card.card_id)}>
        <div className="flex justify-between mb-4">
          ...
          <span className="text-lg">
            <span
              className={`font-bold dark:text-slate-400 ${
                isWarning
                  ? "text-red-500 dark:text-red-700"
                  : "text-green-600 dark:text-green-400"
              }`}
            ...
            {totalBalance > 0 && (
              <span className="dark:text-slate-500"> 남음</span>
            )}
          </span>
        </div>
        <ProgressBars spentPercentage={spentPercentage} isWarning={isWarning} />
          ...
          <div className="flex-1 overflow-y-auto">
            {selectedTransactions.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {Object.entries(
                  groupTransactionsByDate(selectedTransactions)
