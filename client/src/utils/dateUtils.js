...
export const calculateYearsSinceEntry = (dates) => {
  const entryDates = dates
    .filter((date) => date.date_type === "entry")
    .map((date) => new Date(date.date));
  const leaveDates = dates
    .filter((date) => date.date_type === "leave")
    .map((date) => new Date(date.date));
  ...
  const latestEntryDate = entryDates.reduce(
    (latest, date) => (date > latest ? date : latest),
    new Date(0)
  );
  const earliestEntryDate = entryDates.reduce(
    (earliest, date) => (date < earliest ? date : earliest),
    new Date()
  );

  // 가장 최근 퇴사일 (없을 수도 있음)
  const latestLeaveDate = leaveDates.reduce(
    (latest, date) => (date > latest ? date : latest),
    null
  );
  ...
  if (latestLeaveDate && latestLeaveDate > latestEntryDate) {
    ...
    const totalDays = Math.floor(
      (latestLeaveDate - latestEntryDate) / (1000 * 60 * 60 * 24)
    );
    ...
    const firstPeriodDays =
      leaveDates.length > 0
        ? Math.floor(
            (latestLeaveDate - earliestEntryDate) / (1000 * 60 * 60 * 24)
          )
        : 0;

    const secondPeriodDays = Math.floor(
      (now - latestEntryDate) / (1000 * 60 * 60 * 24)
    );
