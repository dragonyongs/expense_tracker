...
export const formatDuration = (start, end, isHalfDay = false) => {
  ...
  if (isHalfDay) {
    ...
    return `${format(
      startDate,
      "yyyy.MM.dd"
    )} ${startTime} ~ ${endTime} ${calculateDuration(startDate, endDate)}`;
  }
  ...
  return `${format(startDate, "yyyy.MM.dd")}(${startDayOfWeek}) ~ ${format(
    endDate,
    "yyyy.MM.dd"
  )}(${endDayOfWeek}) (${dayDifference}일간)`;
};
...
export const StatusLabel = ({ status }) => {
  ...
  return (
    <span
      className={`px-2.5 py-1 text-xs font-medium rounded-lg ${getStatusStyle(
        status
      )}`}
    >
      {status}
...
export const TypeBadge = ({ type }) => {
  ...
  return (
    <span
      className={`px-2.5 py-1 text-xs font-medium rounded-lg ${getTypeStyle(
        type
      )}`}
    >
      {type}
