...
exports.getCardTransactions = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "트랜잭션 조회 중 오류가 발생했습니다." });
  }
};
...
exports.createTransaction = async (req, res) => {
  ...
  if (
    ...
    !transaction_date ||
    !transaction_type ||
    (transaction_type === "expense" && !expense_type && !is_deducted) ||
    !merchant_name ||
    !merchant_name.trim()
  ) {
  ...
  // 헬퍼 함수: 카드의 특정 source(잔액, 팀운영비, 이월금액)에서 금액 차감
  ...
  // 헬퍼 함수: expense 처리 (팀운영비와 일반 지출 처리)
  ...
  // 헬퍼 함수: income 처리 (RegularDeposit, TeamFund, TransportationExpense)
  const handleIncome = (card, menu_items) => {
    ...
    menu_items.forEach(item => {
      ...
      if (deposit_type === "RegularDeposit") {
        ...
        // TransportationExpense는 잔액 업데이트 시 별도 조건 없이 전액 반영
        card.balance += itemAmount;
        totalTransactionAmount += itemAmount;
      }
    });
  ...
  try {
    ...
    if (transaction_type === "expense" || is_deducted) {
      ...
      // 각 메뉴 항목에 대해 expense 처리 수행
      menu_items.forEach(item => {
        const { price, quantity } = item;
        const itemAmount = price * quantity;
        totalTransactionAmount += itemAmount;
        
        const { remainingAmount, teamFundDeducted: deductedTeamFund, rolloverAmounted: deductedRollover } =
          handleExpense(card, expense_type, itemAmount);

        finalRemaining += remainingAmount;
        teamFundDeducted += deductedTeamFund;
        rolloverAmounted += deductedRollover;
      });

      if (finalRemaining > 0) {
        return res.status(400).json({ error: `잔액, 이월 금액 및 팀 운영비가 부족합니다. 부족액: ${finalRemaining} 원` });
      }
      ...
      totalTransactionAmount = handleIncome(card, menu_items);
  ...
  try {
    ...
    if (transaction.transaction_type === "expense") {
      ...
      const remainingToRestore =
        previousAmount -
        (transaction.rolloverAmounted + transaction.teamFundDeducted);
      ...
      if (card.balance < 0 || card.team_fund < 0) {
        return res.status(400).json({
          error:
            "잔액 또는 팀 운영비가 음수가 될 수 없습니다. 데이터 상태를 확인해주세요.",
        });
      }
    }
...
exports.getTransactionsByYearAndMonth = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching transactions by year and month",
        error,
      });
  }
};
...
exports.getAllDeposits = async (req, res) => {
  try {
    ...
    const depositWithMemberNames = await Promise.all(
      deposits.map(async (deposit) => {
        const card = deposit.card_id;
        if (card) {
          const cardMember = await Card.findById(card._id).populate(
            "member_id"
          );

          return {
            ...deposit.toObject(), // deposit 객체를 일반 객체로 변환
            member_name: cardMember.member_id
              ? cardMember.member_id.member_name
              : null, // member_id에서 이름 추출
          };
        }
...
exports.getSearchKeyword = async (req, res) => {
  ...
  try {
    ...
    const merchantCount = transactions.reduce((acc, transaction) => {
      acc[transaction.merchant_name] =
        (acc[transaction.merchant_name] || 0) + 1;
      return acc;
    }, {});
...
exports.getMenuForMerchant = async (req, res) => {
  ...
  try {
    ...
    const uniqueMenus = Object.values(menuCount).sort(
      (a, b) => b.count - a.count
    );
