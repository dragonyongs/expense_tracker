...
const upsertAvatar = async (req, res) => {
    ...
    try {
        ...
        let avatar = await Avatar.findOne({ memberId });

        if (avatar) {
            // 아바타가 있으면 업데이트
            avatar = await Avatar.findOneAndUpdate({ memberId }, req.body, { new: true, runValidators: true });
...
const getAvatarByMemberId = async (req, res) => {
    ...
    try {
        const avatar = await Avatar.findOne({ memberId });
        if (!avatar) {
